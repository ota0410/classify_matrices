...
data_x = np.array(h5py.File("/home/s1411506/ota/kfold/algorithm.h5",'r').get('data_x'))
label_y = np.array(h5py.File("/home/s1411506/ota/kfold/algorithm.h5",'r').get('label_y'))
...
if ckpt and len(args) == 2:
    ...
    prediction = y_p
    #print("Prediction for BiCG "+ args[1] + ":%g"%prediction.eval(feed_dict={x: new_img, keep_prob: 1.0}, session=sess))
    #print("%g"%prediction.eval(feed_dict={x: new_img, keep_prob: 1.0}, session=sess))
    x = prediction.eval(feed_dict={x: new_img, keep_prob: 1.0}, session=sess)
    if(bool(x)):
        print('Use BiCG Method')
    else:
        print('Use CGNR Method')
else:
    loss1 = []
    loss2 = []
    percentage = []
